...
export default function SettingsPage() {
  ...
  async function createKeyUser(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault()
    const form = event.currentTarget
    const data = new FormData(form)
    setError(null); setGeneratedKey(null)
    try {
      const result = await api.admin.createKeyUser({ name: String(data.get('name')), role: String(data.get('role')) as UserRole, label: String(data.get('label') || 'initial') })
      setGeneratedKey(result.access_key); form.reset(); await loadUsers()
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Access key generation failed') }
  }
  ...
  return <PlaylistShell>
    ...
    {!draft ? <FuzeState kind="loading">LOADING SETTINGS…</FuzeState> : <div className="fuze-section-grid">
      <Section title="General"><label className="fuze-field">Instance name<input className={inputClass} value={draft.instance_name} maxLength={100} onChange={(event) => setDraft({ ...draft, instance_name: event.target.value })} /></label><p className="fuze-admin-meta">Configuration version {draft.version} · application {system?.app_version ?? '…'} · schema {system?.schema_revision ?? '…'}</p></Section>
      <Section title="Authentication"><label className="fuze-field">Login methods<select className={inputClass} value={draft.auth.mode} onChange={(event) => setDraft({ ...draft, auth: { ...draft.auth, mode: event.target.value as AdminSettings['auth']['mode'] } })}><option value="password">Password</option><option value="key">Access key</option><option value="both">Password and access key</option></select></label><label className="fuze-check"><input type="checkbox" checked={draft.auth.registration} disabled={draft.auth.mode === 'password'} onChange={(event) => setDraft({ ...draft, auth: { ...draft.auth, registration: event.target.checked } })} />Allow public registration</label></Section>
      <Section title="Providers">
        {(['youtube', 'yandex', 'spotify'] as const).map((provider) => <div key={provider} className="fuze-admin-provider"><div className="fuze-admin-row"><label className="fuze-check fuze-admin-grow"><input type="checkbox" checked={draft.providers[provider]} onChange={(event) => setDraft({ ...draft, providers: { ...draft.providers, [provider]: event.target.checked } })} />{provider}</label><button type="button" className={buttonClass} onClick={() => void testProvider(provider)}>Test connection</button></div>{providerTests[provider] && <p className="fuze-admin-meta">{providerTests[provider].message} {providerTests[provider].latency_ms ? `(${providerTests[provider].latency_ms} ms)` : ''}</p>}</div>)}
      ...
      <Section title="Users"><form onSubmit={createUser} className="grid gap-3 sm:grid-cols-4"><input required name="name" placeholder="Name" className={inputClass} /><input required name="email" type="email" placeholder="Email" className={inputClass} /><input required name="password" type="password" minLength={8} placeholder="Initial password" className={inputClass} /><div className="flex gap-2"><select name="role" className={inputClass}><option value="user">User</option><option value="admin">Admin</option></select><button className={buttonClass} type="submit">Create</button></div></form><form onSubmit={createKeyUser} className="grid gap-3 rounded-lg border border-border p-3 sm:grid-cols-4"><input required name="name" placeholder="Key user name" className={inputClass} /><input name="label" placeholder="Key label (initial)" className={inputClass} /><select name="role" className={inputClass}><option value="user">User</option><option value="admin">Admin</option></select><button className={buttonClass} type="submit">Generate access key</button></form>{generatedKey && <div role="status" className="rounded-lg border border-green-500/30 bg-green-500/10 p-4"><p className="text-sm font-medium">Access key generated. It is shown only once.</p><code className="mt-2 block select-all break-all rounded bg-bg p-3 text-xs">{generatedKey}</code><button type="button" className={`${buttonClass} mt-3`} onClick={() => void navigator.clipboard.writeText(generatedKey)}>Copy key</button></div>}<input aria-label="Search users" value={search} onChange={(event) => { setSearch(event.target.value); setUserPage(1) }} placeholder="Search users" className={inputClass} /><div className="divide-y divide-border">{users.map((user) => <div key={user.id} className="flex flex-wrap items-center gap-3 py-3 text-sm"><div className="min-w-48 flex-1"><div className="font-medium">{user.name}</div><div className="text-xs text-text-muted">{user.email ?? 'Access key user'}</div></div><select value={user.role} className="h-9 rounded-lg bg-hover-strong px-2" onChange={(event) => void updateUser(user, { role: event.target.value as UserRole })}><option value="user">User</option><option value="admin">Admin</option></select><button className={buttonClass} onClick={() => void updateUser(user, { is_active: !user.is_active })}>{user.is_active ? 'Deactivate' : 'Activate'}</button>{user.email && <button className={buttonClass} onClick={() => { const password = window.prompt(`New password for ${user.email}`); if (password) void updateUser(user, { password }) }}>Reset password</button>}</div>)}</div><div className="flex items-center justify-between text-xs text-text-muted"><span>{userTotal} users</span><div className="flex gap-2"><button className={buttonClass} disabled={userPage <= 1} onClick={() => setUserPage((value) => value - 1)}>Previous</button><button className={buttonClass} disabled={userPage * 20 >= userTotal} onClick={() => setUserPage((value) => value + 1)}>Next</button></div></div></Section>
      ...
      <div className="fuze-admin-savebar"><p>{changes.length ? `Pending: ${changes.join(', ')}` : 'No pending changes'}</p><button type="button" disabled={busy || changes.length === 0} onClick={() => void save()} className="fuze-button fuze-button--primary">{busy ? 'Saving…' : 'Save changes'}</button></div>
    </div>}
  </PlaylistShell>
}
