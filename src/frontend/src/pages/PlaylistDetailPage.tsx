...
import { ArrowDown, ArrowLeft, ArrowUp, DownloadSimple, MusicNotes, PencilSimple, Play, Plus, Spinner, Trash, X } from '@phosphor-icons/react'
...
export default function PlaylistDetailPage() {
  ...
  return <PlaylistShell>
    ...
    : <>
      ...
      : <ol className="fuze-track-list" aria-label="Playlist tracks">{playlist.items.map((item, index) => <li key={item.id} className="fuze-track-row"><span className="fuze-track-row__index">{index + 1}</span><button type="button" disabled={!config?.features.playback} onClick={() => play(item)} className="fuze-track-row__main" aria-label={config?.features.playback ? `Play ${item.track.title}` : `${item.track.title} metadata`}><span className="fuze-track-row__art">{item.track.cover_url ? <img src={item.track.cover_url} alt="" /> : <MusicNotes />}</span><span className="fuze-track-row__copy"><b>{item.track.title}</b><small>{item.track.artist}</small></span></button><time>{duration(item.track.duration_ms)}</time><div className="fuze-track-row__actions"><FuzeButton variant="icon" disabled={itemBusy !== null || item.track.download_status !== 'ready'} onClick={() => void downloadTrack(item.track)} aria-label={`Download ${item.track.title}`}><DownloadSimple /></FuzeButton><FuzeButton variant="icon" disabled={index === 0 || itemBusy !== null} onClick={() => void move(item.id, -1)} aria-label={`Move ${item.track.title} up`}><ArrowUp /></FuzeButton><FuzeButton variant="icon" disabled={index === playlist.items.length - 1 || itemBusy !== null} onClick={() => void move(item.id, 1)} aria-label={`Move ${item.track.title} down`}><ArrowDown /></FuzeButton><FuzeButton variant="icon" disabled={itemBusy !== null} onClick={() => void removeItem(item.id)} aria-label={`Remove ${item.track.title}`}><X /></FuzeButton></div></li>)}</ol>}
